 ...
 * v3: 기획서 기반 DeepStock 소개 섹션 추가, 수수료 0.05%
 */
"use client";
...
const PROFILES = [
  ...
  },
  {
    ...
    desc: "성장 섹터 집중. 나스닥·테크 섹터 비중을 극대화하여 장기 고수익을 추구합니다. 변동성을 감수할 수 있는 투자자에게 적합.",
...
const AI_FEATURES = [
  { icon: "🧠", title: "마켓 레짐 감지", desc: "VIX, 금리 스프레드, 이동평균 배열을 종합 분석해 현재 시장이 '강세/약세/위기/횡보' 중 어떤 국면인지 자동 판단합니다." },
  { icon: "📰", title: "감성 분석 (NLP)", desc: "FinBERT AI가 매일 주요 금융 뉴스를 읽고 시장 심리를 점수화합니다. 부정 감성 급등 시 방어 포지션으로 자동 전환." },
  { icon: "🔗", title: "크로스에셋 분석", desc: "주식·채권·금·달러 간 상관관계가 '깨지는 순간'을 포착합니다. 자산 간 이상 패턴 감지 시 즉시 리밸런싱을 트리거합니다." },
  { icon: "📡", title: "얼터너티브 데이터", desc: "Fear & Greed Index, Google Trends 검색량, 원/달러 환율 급변 등 비전통 데이터로 전통 지표가 놓치는 시장 신호를 포착합니다." },
];
  ...
  return (
    <div style={{ minHeight: "100vh", overflow: "hidden" }}>
      <style>{`
        ...
        .step-line { position:absolute; top:20px; left:calc(50% + 20px); width:calc(100% - 40px); height:1px; background:rgba(255,255,255,0.06); }
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 100px" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <div style={{
            ...
            fontSize: 12, color: "#FF6B35", fontWeight: 600, marginBottom: 16, letterSpacing: 1,
          }}>DEEPSTOCK · 전략 프로파일</div>
          <h2 style={{ fontSize: 32, fontWeight: 700, color: "#e8e8ed", marginBottom: 12 }}>위험도만 선택하세요</h2>
          <p style={{ fontSize: 15, color: "#6b6b7e", maxWidth: 480, margin: "0 auto" }}>
            3가지 프로파일 중 하나를 선택하면 AI가 시장 상황에 맞게 ETF 비중을 동적으로 조절합니다
          </p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
          {PROFILES.map((p) => (
            <div key={p.name} className="profile-card" style={{
              borderRadius: 20, padding: 32,
              background: p.highlight ? "rgba(255,107,53,0.04)" : "rgba(255,255,255,0.02)",
              borderColor: p.highlight ? "rgba(255,107,53,0.2)" : "rgba(255,255,255,0.06)",
              position: "relative", overflow: "hidden",
            }}>
              {p.highlight && (
                <div style={{
                  position: "absolute", top: 16, right: 16, padding: "3px 10px",
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
                <div style={{
                  ...
                  <span style={{ fontSize: 16, fontWeight: 700, color: p.color }}>{p.target}</span>
                </div>
                <div style={{
                  padding: "6px 14px", borderRadius: 8,
                  background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)",
                }}>
                  <span style={{ fontSize: 13, color: "#6b6b7e" }}>{p.mdd}</span>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {p.tags.map((t) => (
                  <span key={t} style={{
                    ...
                    background: "rgba(255,255,255,0.04)", color: "#6b6b7e",
                    border: "1px solid rgba(255,255,255,0.06)",
                  }}>{t}</span>
                ))}
              </div>
        ...
        <p style={{ textAlign: "center", fontSize: 12, color: "#4a4a5e", marginTop: 20 }}>
          ※ 레버리지 ETF는 모든 프로파일에서 제외됩니다. 과거 시뮬레이션 수익률은 미래 수익을 보장하지 않습니다.
        </p>
      </section>
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 100px" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <div style={{
            ...
            background: "rgba(255,107,53,0.06)", border: "1px solid rgba(255,107,53,0.15)",
            fontSize: 12, color: "#FF6B35", fontWeight: 600, marginBottom: 16, letterSpacing: 1,
          }}>DEEPSTOCK · AI 시장분석 엔진</div>
          <h2 style={{ fontSize: 32, fontWeight: 700, color: "#e8e8ed", marginBottom: 12 }}>단순 RSI가 아닙니다</h2>
          <p style={{ fontSize: 15, color: "#6b6b7e", maxWidth: 480, margin: "0 auto" }}>
            최신 AI 전략 4가지를 복합 적용하여 시장 상황을 종합 판단하고 포트폴리오를 자동 조절합니다
          </p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20 }}>
          {AI_FEATURES.map((f, i) => (
            <div key={i} className="ai-card" style={{
              background: "rgba(255,255,255,0.02)", borderRadius: 16, padding: 28,
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 100px" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <div style={{
            ...
            background: "rgba(255,107,53,0.06)", border: "1px solid rgba(255,107,53,0.15)",
            fontSize: 12, color: "#FF6B35", fontWeight: 600, marginBottom: 16, letterSpacing: 1,
          }}>DEEPSTOCK · 수수료 정책</div>
          <h2 style={{ fontSize: 32, fontWeight: 700, color: "#e8e8ed", marginBottom: 12 }}>투명한 수수료</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{
            ...
            <div style={{
              textAlign: "center", padding: "24px 0", marginBottom: 24,
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 100px" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <h2 style={{ fontSize: 32, fontWeight: 700, color: "#e8e8ed", marginBottom: 12 }}>이렇게 시작하세요</h2>
